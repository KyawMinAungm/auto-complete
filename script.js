...
autoCompleteInputTag.addEventListener("keyup", (event) => {
  if (event.key === "ArrowDown" ||
      event.key === "ArrowUp" ||
      event.key === "Enter") {
    navigateAndSelectProduct(event);
  }
  ...
  return product.title.toLowerCase().includes(enterText);
  });
for (let i = 0; i < filteredProducts.length; i++) {
  ...
  productItemContainer.id = product.id;
...
let indexToSelect = -1;
const navigateAndSelectProduct = (keys) => {
  if (keys.key === "ArrowDown") {
    indexToSelect += 1;
